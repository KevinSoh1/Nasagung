...
// 1. 이미 로그인된 경우 메인으로 이동
if (isset($_SESSION['user_email'])) {
    header("Location: mypage.php");
    exit;
}

// 2. 소셜 로그인 URL 생성 (register.php와 동일한 설정)
$naver_client_id = 'UF79Ckgq9nduIA5W44JW';
$naver_redirect  = urlencode('https://highteck.kr/nasagung/callback.php?type=naver');
//$kakao_client_id = '9f06bbc7ab94242556b69913773c35db';
$kakao_client_id = '3df972d36e9c33e81dc306fa0829de88';
$kakao_redirect  = urlencode('https://highteck.kr/nasagung/callback.php?type=kakao');
...
$naver_url = "https://nid.naver.com/oauth2.0/authorize?response_type=code&client_id=" . $naver_client_id . "&redirect_uri=" . $naver_redirect . "&state=" . $_SESSION['naver_state'];
$kakao_url = "https://kauth.kakao.com/oauth/authorize?client_id=" . $kakao_client_id . "&redirect_uri=" . $kakao_redirect . "&response_type=code";
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    // DB에서 해당 이메일의 로컬 계정 정보 조회
    $sql = "SELECT * FROM nasagung_users WHERE email = '$email' AND provider = 'local'";
    $result = mysqli_query($conn, $sql);

    if ($row = mysqli_fetch_assoc($result)) {
        // 비밀번호 검증 (암호화 방식에 따라 password_verify 또는 md5 사용)
        $is_pw_correct = false;
        if (function_exists('password_verify')) {
            $is_pw_correct = password_verify($pw, $row['password']);
        } else {
            $is_pw_correct = (md5($pw) === $row['password']);
        }
        ...
        if ($is_pw_correct) {
            ...
            $_SESSION['currentPoint']  = $row['current_point'];
            $_SESSION['user_type']  = 'local';

            echo "<script>alert('" . $row['name'] . "님, 환영합니다!'); location.href='mypage.php';</script>";
            exit;
        } else {
            $msg = "<div class='alert alert-danger'>비밀번호가 일치하지 않습니다.</div>";
        }
    } else {
        $msg = "<div class='alert alert-danger'>존재하지 않는 계정이거나 가입 경로가 다릅니다.</div>";
    }
}
?>
    ...
    <style>
  	...
  	a:active {
        ...
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        /* register.php의 .reg-card 규격과 완전 일치하도록 동기화 */
        .login-card {
            max-width: 450px;     /* 가독성을 위해 로그인 화면에 최적화된 너비 유지 */
            width: 100%;
            margin: 0 auto;       /* 가로 정중앙 정렬 */
            padding: 2.5rem 2rem;
            border-radius: 15px;
            border: none;
            box-shadow: 0 10px 30px rgba(0,0,0,0.1);
            background: #fff;
            color: #333 !important; /* 부트스트랩 흰 글씨 오버라이딩 차단 */
        }

        /* 로그인 카드 내부 디자인 가독성 확보 */
        .login-card h3 { color: #333 !important; font-weight: 600; }
        .login-card label { color: #555; margin-bottom: 5px; margin-top: 10px; font-weight: bold; }
        .login-card label:first-of-type { margin-top: 0; }
        .login-card .text-muted { color: #6c757d !important; }

        /* 소셜 간편 로그인 버튼 스타일 정의 */
        .btn-naver { background: #03C75A; color: white !important; }
        ...
        .input-group input { 
            width: 100%; padding: 14px 18px; border: 1px solid #d1ccc0; 
            border-radius: 4px; font-size: 1rem; background-color: #f7f3eb;
        }
    </style>
</head>
<body>
...
<main class="hero">
    <div class="hero-content">
        ...
        <div class="card login-card p-4 mt-5">
            <h3 class="text-center fw-bold mb-4">나사궁 로그인</h3>
            <?php echo $msg; ?>
            <form action="login.php" method="POST" class="mb-4">
                ...
                <button type="submit" class="btn btn-primary w-100 fw-bold py-2">로그인</button>
            </form>

            <div class="text-center small text-muted mb-3">또는 간편 로그인</div>
        
            <a href="<?php echo $naver_url; ?>" class="btn-social btn-naver">네이버로 시작하기</a>
            <a href="<?php echo $kakao_url; ?>" class="btn-social btn-kakao">카카오로 시작하기</a>
            
            <div class="text-center mt-4 small">
                계정이 없으신가요? <a href="register.php" class="text-decoration-none text-primary fw-bold">회원가입</a>
            </div>
        </div>
    </div>
</main>
