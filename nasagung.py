...
# ==========================================
# 6. 로또 번호 예측 페이지 (GET & POST)
# ==========================================
@app.api_route("/lotto", methods=["GET", "POST"], response_class=HTMLResponse)
async def lotto_page(
    request: Request,
    name: str = Form(None),
    birthYear: str = Form(None),
    birthDay: str = Form(None),
    birthTime: str = Form(None),
    gender: str = Form(None),
    calendarType: str = Form(None),
    fiveElements: str = Form(None)
):
    # GET 요청으로 직접 접속했을 때 (폼 제출이 아닌 경우)
    if request.method == "GET":
        return templates.TemplateResponse(
            "lotto.html",
            {
                "request": request,
                "result": None,
                "user": None  # 필요한 경우 세션에서 유저 정보 전달
            }
        )

    # POST 요청일 때 (폼을 제출하여 번호를 생성하는 경우)
    # 1. OpenAI 호출 로직 수행
    # prompt = create_lotto_prompt(...)
    # result_text = call_openai_api(prompt)
    
    # 예시 결과 반환
    result_text = "제 1게임: 05, 12, 23, 31, 38, 44\n..." 
